...
class YcbineoatReader:
  def __init__(self,video_dir, downscale=1, shorter_side=None, zfar=np.inf):
    ...
    metadata_path = os.path.join(video_dir, "metadata_new.csv")
    self.metadata = {}
    with open(metadata_path, "r") as file:
      reader = csv.DictReader(file)
      for row in reader:
          file_name = row["file_name"]
          cam_K = json.loads(row["cam_K"])
  ...
  def get_K(self, i):
    file_name = os.path.basename(self.color_files[i])
    return self.metadata[file_name]["cam_K"]
  ...
  def get_mask(self,i):
    mask = cv2.imread(self.color_files[i].replace('rgb','mask_visib'),-1)
    if len(mask.shape)==3:
      for c in range(3):
        if mask[...,c].sum()>0:
          mask = mask[...,c]
